...
const NAV = [
  ...
  { to: "/admin/overview", label: "Platform overview", icon: Globe2 },
  { to: "/admin/users", label: "User management", icon: Users },
  { to: "/admin/government", label: "Government management", icon: ShieldCheck },
  { to: "/admin/business", label: "Business management", icon: Building2 },
  ...
  { to: "/admin/rules", label: "Legal Metrology rules", icon: Gavel },
];

function AdminLayout() {
  const pathname = useRouterState({ select: (s) => s.location.pathname });
  const isLogin = pathname === "/admin/login";
  const { loading, session, profile, isMainAdmin, signOut } = useAuth();
  ...
  if (loading || !session || !isMainAdmin) {
    return (
  ...
  return (
    <div className="min-h-screen bg-background lg:flex">
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        ...
        <main className="safe-px safe-pb mx-auto w-full max-w-7xl flex-1 py-6 sm:py-8">
          <Outlet />
        </main>
      </div>
    </div>
